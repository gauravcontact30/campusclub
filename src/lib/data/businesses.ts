...
import { isOpenNow, slugify } from '@/lib/utils';
...
function applyDemoQuery(query: BusinessQuery): Paginated<Business> {
  const { term, city, category, price, minRating, openNow, sort = 'recommended' } = query;
  const page = Math.max(1, query.page ?? 1);
  const perPage = query.perPage ?? DEFAULT_PER_PAGE;

  let items = db().businesses.map((b) => withAggregates(b));
...
export async function searchBusinesses(query: BusinessQuery = {}): Promise<Paginated<Business>> {
  ...
  q = q.order(column, { ascending }).range((page - 1) * perPage, page * perPage - 1);

  const { data, count, error } = await q;
  if (error) throw new Error(error.message);

  let items = (data ?? []).map(fromRow);
  // `open now` depends on the visitor's clock, so it stays in application code.
