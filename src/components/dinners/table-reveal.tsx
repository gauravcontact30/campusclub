...
export function TableReveal({ mates, locked }: { mates: TableMate[]; locked: boolean }) {
  return (
    <section className="rounded-4xl bg-noir p-7 text-pearl sm:p-9">
      <div className="flex items-center gap-2">
        <Sparkles size={18} className="text-rouge" />
        <h2 className="font-display text-2xl font-semibold">Your table</h2>
      </div>
      <p className="mt-2 max-w-xl text-sm text-pearl/65">
        {locked
      ...
      <ul className="mt-7 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {mates.map((mate, i) => (
          <li
            key={`${mate.name}-${i}`}
            className="flex items-center gap-3 rounded-2xl border border-pearl/15 bg-pearl/5 p-4"
          >
            ...
            <div className={locked ? 'blur-[5px] select-none' : ''}>
              ...
              <p className="text-xs text-pearl/60">{mate.works}</p>
