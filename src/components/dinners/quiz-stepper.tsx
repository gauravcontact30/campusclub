...
export function QuizStepper({ signedIn, initialAnswers }: { signedIn: boolean; initialAnswers: Record<string, string> }) {
  ...
  if (done) {
    return (
      <div className="surface-card p-8 text-center sm:p-12">
        <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-parrot/25 text-frost">
          <Check size={30} />
        </span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between text-sm text-frost/60">
        <span>
      ...
      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-frost/10">
        <div className="h-full rounded-full bg-orchid transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>

      <div key={question.id} className="mt-9 animate-fade-up">
        <h2 className="display-md">{question.prompt}</h2>
        <p className="mt-2 text-sm text-frost/60">{question.help}</p>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? 'border-orchid bg-orchid text-frost' : 'border-frost/15 bg-noir-700 hover:border-frost/40',
                )}
              >
                <span className="flex items-center justify-between gap-3">
                  ...
                  <span
                    className={cn(
                      ...
                      active ? 'border-orchid bg-orchid text-frost' : 'border-frost/25',
                    )}
                  >
                ...
                {'hint' in option && option.hint && (
                  <span className={cn('mt-1.5 block text-sm', active ? 'text-frost/60' : 'text-frost/60')}>
                    {option.hint}
                  </span>
                )}
              </button>
      ...
      <div className="mt-9 flex items-center justify-between gap-3">
        ...
        {index < total - 1 ? (
          ...
          <Link
            ...
            className="inline-flex h-14 items-center rounded-full bg-orchid px-8 font-semibold text-frost hover:bg-orchid-600"
          >
        ...
        {QUIZ_QUESTIONS.map((q, i) => (
          <button
            ...
            className={cn(
              ...
              i === index ? 'w-8 bg-frost' : (answers[q.id] ?? merged[q.id]) ? 'w-4 bg-orchid' : 'w-4 bg-frost/15',
            )}
          />
        ))}
      </div>
