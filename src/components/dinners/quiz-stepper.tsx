  ...
  if (done) {
    return (
      <div className="surface-card p-8 text-center sm:p-12">
        <span className="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-sage/25 text-ink">
          <Check size={30} />
        </span>
  ...
  return (
    <div>
      ...
      <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-ink/10">
        <div className="h-full rounded-full bg-flame transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>

      <div key={question.id} className="mt-9 animate-fade-up">
        <h2 className="display-md">{question.prompt}</h2>
        <p className="mt-2 text-sm text-ink/55">{question.help}</p>
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  active ? 'border-ink bg-ink text-cream' : 'border-ink/15 bg-cream-100 hover:border-ink/40',
                )}
              >
                <span className="flex items-center justify-between gap-3">
                  ...
                  <span
                    className={cn(
                      ...
                      active ? 'border-flame bg-flame text-cream' : 'border-ink/25',
                    )}
                  >
                ...
                {'hint' in option && option.hint && (
                  <span className={cn('mt-1.5 block text-sm', active ? 'text-cream/60' : 'text-ink/55')}>
                    {option.hint}
                  </span>
                )}
              </button>
      ...
      <div className="mt-9 flex items-center justify-between gap-3">
        ...
        {index < total - 1 ? (
          ...
          <Link
            ...
            className="inline-flex h-14 items-center rounded-full bg-flame px-8 font-semibold text-cream hover:bg-flame-600"
          >
        ...
        {QUIZ_QUESTIONS.map((q, i) => (
          <button
            ...
            className={cn(
              ...
              i === index ? 'w-8 bg-ink' : (answers[q.id] ?? merged[q.id]) ? 'w-4 bg-flame' : 'w-4 bg-ink/15',
            )}
          />
        ))}
      </div>
