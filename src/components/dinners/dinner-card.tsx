    ...
    <article className="surface-card flex flex-col gap-5 p-6 transition-all hover:-translate-y-1 hover:shadow-lift">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-2xl border border-content/12 bg-canvas-600 text-content">
            <span className="text-[10px] font-semibold uppercase tracking-widest text-content/60">
          ...
          <div>
            ...
            <p className="mt-1 flex items-center gap-1.5 text-sm text-content/60">
              <Clock size={14} />
        ...
        {seatsLeft <= 2 && seatsLeft > 0 && <Badge tone="rouge">{pluralize(seatsLeft, 'seat')} left</Badge>}
        {seatsLeft === 0 && <Badge tone="neutral">Waitlist</Badge>}
      </div>

      {!compact && (
        <p className="flex items-center gap-1.5 text-sm text-content/60">
      ...
      <div>
        <div className="flex items-center justify-between text-xs font-medium text-content/60">
        ...
        <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-content/10">
          <div className="h-full rounded-full bg-rouge transition-all" style={{ width: `${filled}%` }} />
        </div>
      </div>

      <div className="mt-auto flex items-center justify-between gap-3 border-t border-content/10 pt-4">
        <p className="text-sm">
          <span className="font-display text-lg font-semibold">{formatMoneyForCity(event.priceCents, event.city)}</span>
          <span className="text-content/55"> / seat</span>
        </p>
