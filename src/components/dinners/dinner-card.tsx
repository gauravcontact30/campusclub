    ...
    <article className="surface-card flex flex-col gap-5 p-6 transition-all hover:-translate-y-1 hover:shadow-lift">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="flex h-16 w-16 shrink-0 flex-col items-center justify-center rounded-2xl bg-ink text-cream">
            <span className="text-[10px] font-semibold uppercase tracking-widest text-cream/60">
        ...
        {seatsLeft <= 2 && seatsLeft > 0 && <Badge tone="flame">{pluralize(seatsLeft, 'seat')} left</Badge>}
      ...
      <div>
        ...
        <div className="mt-2 h-1.5 w-full overflow-hidden rounded-full bg-ink/10">
          <div className="h-full rounded-full bg-flame transition-all" style={{ width: `${filled}%` }} />
        </div>
      </div>
      ...
      <div className="mt-auto flex items-center justify-between gap-3 border-t border-ink/10 pt-4">
        <p className="text-sm">
          ...
          <span className="text-ink/50"> / seat</span>
        </p>
