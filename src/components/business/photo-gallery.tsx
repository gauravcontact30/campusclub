    ...
    <>
      <div className="grid gap-2 overflow-hidden rounded-4xl sm:grid-cols-4 sm:grid-rows-2">
        {shots.slice(0, 5).map((src, i) => (
          <button
            ...
            onClick={() => setOpen(i)}
            className={cn(
              'relative overflow-hidden bg-content/5 transition-opacity hover:opacity-90',
      ...
      {open !== null && (
        <div
          ...
          className="fixed inset-0 z-[70] flex animate-fade-in items-center justify-center bg-canvas/95 p-4"
          onClick={() => setOpen(null)}
        >
          <button
            onClick={() => setOpen(null)}
            aria-label="Close photo viewer"
            className="absolute right-5 top-5 flex h-11 w-11 items-center justify-center rounded-full bg-content/10 text-content hover:bg-content/20"
          >
            <X size={20} />
          </button>
