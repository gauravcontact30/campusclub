...
export function HoursTable({ hours }: { hours: WeekHours }) {
  ...
  return (
    <dl className="divide-y divide-pearl/10">
      {WEEKDAYS.map((day, i) => (
        <div
          key={day}
          className={cn('flex items-center justify-between py-2.5 text-sm', today === i && 'font-semibold text-pearl')}
        >
          <dt className={today === i ? '' : 'text-pearl/60'}>{day}</dt>
          <dd className={hours[i].open ? '' : 'text-pearl/50'}>
