  ...
  return (
    <dl className="divide-y divide-content/10">
      {WEEKDAYS.map((day, i) => (
        <div
          key={day}
          className={cn('flex items-center justify-between py-2.5 text-sm', today === i && 'font-semibold text-content')}
        >
          <dt className={today === i ? '' : 'text-content/60'}>{day}</dt>
          <dd className={hours[i].open ? '' : 'text-content/50'}>
