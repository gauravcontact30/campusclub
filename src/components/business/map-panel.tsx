...
export function MapPanel({ business }: { business: Business }) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-ink/10">
      <div className="relative h-44 bg-ink-600">
        <svg aria-hidden viewBox="0 0 400 180" className="absolute inset-0 h-full w-full opacity-40">
          <defs>
            <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M40 0H0V40" fill="none" stroke="#F8F4EA" strokeWidth="0.6" />
            </pattern>
          </defs>
          <rect width="400" height="180" fill="url(#grid)" />
          <path d="M0 120 L140 120 L140 180" stroke="#A8C3B0" strokeWidth="6" fill="none" opacity="0.5" />
          <path d="M240 0 L240 90 L400 90" stroke="#A8C3B0" strokeWidth="6" fill="none" opacity="0.5" />
        </svg>
        <span className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-flame text-cream shadow-lift">
            <MapPin size={20} />
          </span>
          <span className="mt-1 h-3 w-3 rotate-45 rounded-sm bg-flame/40" />
        </span>
      </div>

      <div className="space-y-1 bg-cream-100 p-5">
        ...
        <a
          ...
          className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-flame-700 hover:underline"
        >
