...
export function MapPanel({ business }: { business: Business }) {
  ...
  return (
    <div className="overflow-hidden rounded-3xl border border-frost/10">
        ...
        <svg aria-hidden viewBox="0 0 400 180" className="absolute inset-0 h-full w-full opacity-40">
          <defs>
            <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
              <path d="M40 0H0V40" fill="none" stroke="#F2EDFB" strokeWidth="0.6" />
            </pattern>
          </defs>
          <rect width="400" height="180" fill="url(#grid)" />
          <path d="M0 120 L140 120 L140 180" stroke="#4ADE64" strokeWidth="6" fill="none" opacity="0.5" />
          <path d="M240 0 L240 90 L400 90" stroke="#4ADE64" strokeWidth="6" fill="none" opacity="0.5" />
        </svg>
        <span className="absolute left-1/2 top-1/2 flex -translate-x-1/2 -translate-y-1/2 flex-col items-center">
          <span className="flex h-11 w-11 items-center justify-center rounded-full bg-orchid text-frost shadow-lift">
            <MapPin size={20} />
          </span>
          <span className="mt-1 h-3 w-3 rotate-45 rounded-sm bg-orchid/40" />
        </span>
      </div>
      ...
      <div className="space-y-1 bg-noir-700 p-5">
        ...
        <a
          ...
          className="mt-3 inline-flex items-center gap-1.5 text-sm font-semibold text-orchid-700 hover:underline"
        >
