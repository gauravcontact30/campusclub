...
export function BusinessCard({
  ...
  return (
    <article className="group surface-card relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lift">
      <div className="relative aspect-[4/3] overflow-hidden bg-frost/5">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-display text-lg font-semibold leading-snug">
            <Link href={`/businesses/${business.slug}`} className="after:absolute after:inset-0 hover:text-orchid">
              {business.name}
            </Link>
          </h3>
          <span className="shrink-0 text-sm font-semibold text-frost/55">{priceLabel(business.priceLevel, business.city)}</span>
        </div>
        ...
        <p className="mt-2 flex items-center gap-1.5 text-sm text-frost/60">
        ...
        <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-frost/65">{business.description}</p>
