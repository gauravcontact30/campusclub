      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-display text-lg font-semibold leading-snug">
            <Link href={`/businesses/${business.slug}`} className="after:absolute after:inset-0 hover:text-flame">
              {business.name}
            </Link>
          </h3>
          <span className="shrink-0 text-sm font-semibold text-ink/50">{priceLabel(business.priceLevel, business.city)}</span>
        </div>
        ...
        <p className="mt-3 line-clamp-2 text-sm leading-relaxed text-ink/65">{business.description}</p>
