...
export function OwnerReply({
  ...
  if (response) {
    return (
      <div className="mt-4 rounded-2xl border-l-2 border-flame bg-ink/[0.04] p-4">
        <p className="flex flex-wrap items-center gap-2 text-sm font-semibold">
          <Store size={15} className="text-flame" />
          Response from {businessName}
          {respondedAt && <span className="font-normal text-xs text-ink/45">{relativeTime(respondedAt)}</span>}
        </p>
        <p className="mt-2 whitespace-pre-line text-sm leading-relaxed text-ink/75">{response}</p>

        {canRespond && (
          <div className="mt-3 flex gap-3">
            <button onClick={() => setOpen((o) => !o)} className="text-xs font-semibold text-flame-700 hover:underline">
              Edit reply
            </button>
            <button
              onClick={withdraw}
              disabled={withdrawing}
              className="inline-flex items-center gap-1 text-xs font-semibold text-ink/50 hover:text-flame"
            >
    ...
    <div className="mt-4">
      {open ? (
        ...
        <button
          ...
          className="inline-flex items-center gap-2 rounded-full border border-ink/15 px-4 py-2 text-xs font-semibold hover:border-ink"
        >
