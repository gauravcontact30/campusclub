...
export function ReviewCard({
  ...
  return (
    <article className="border-b border-ink/10 py-7 last:border-0">
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <Avatar name={review.authorName} src={review.authorAvatar} size={44} />
          <div>
            <p className="font-semibold">{review.authorName}</p>
            <p className="text-xs text-ink/50">{relativeTime(review.createdAt)}</p>
          </div>
        </div>
        {isOwn && (
          <button
            onClick={remove}
            disabled={pending}
            className="inline-flex items-center gap-1.5 rounded-full border border-ink/15 px-3 py-1.5 text-xs font-medium text-ink/60 hover:border-flame hover:text-flame"
          >
      ...
      <p className="mt-2 whitespace-pre-line text-sm leading-relaxed text-ink/75">{review.body}</p>

      {review.photos.length > 0 && (
        <div className="mt-4 flex gap-2">
          {review.photos.map((photo, i) => (
            <div key={photo + i} className="relative h-24 w-32 overflow-hidden rounded-2xl bg-ink/5">
      ...
      <button
        ...
        disabled={pending}
        aria-pressed={voted}
        className="mt-4 inline-flex items-center gap-2 rounded-full border border-ink/15 px-4 py-2 text-xs font-semibold transition-colors hover:border-ink data-[voted=true]:border-flame data-[voted=true]:text-flame"
        data-voted={voted}
      >
        <ThumbsUp size={14} />
        Helpful ({count})
