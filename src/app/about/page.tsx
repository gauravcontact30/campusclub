...
export const metadata: Metadata = {
  title: 'About',
  description: 'Why HomeMart exists: better local recommendations, and somebody to use them with.',
};
    ...
    <>
      <section className="container-page py-16 sm:py-24">
        <div className="max-w-3xl">
          <p className="eyebrow">About HomeMart</p>
          ...
          <p className="lede mt-6">
            ...
            good at telling you where to eat and terrible at giving you someone to eat with. HomeMart does both, and
      ...
      <section className="bg-ink py-20 text-cream">
        <div className="container-page grid gap-12 lg:grid-cols-[0.9fr_1.1fr]">
          <div>
            <h2 className="display-lg text-cream">How we got here</h2>
            <p className="lede mt-4 text-cream/70">
              No growth hacks, no fake reviews, no paid placement. Just a table that keeps getting longer.
            </p>
          </div>
          <ol className="space-y-6">
            {MILESTONES.map(([year, text]) => (
              <li key={year} className="flex gap-6 border-b border-cream/15 pb-6 last:border-0">
                <span className="w-20 shrink-0 font-display text-xl font-semibold text-flame">{year}</span>
                <span className="text-sm leading-relaxed text-cream/75">{text}</span>
              </li>
            ))}
          </ol>
        </div>
      ...
      <section className="container-page py-20">
        ...
        <div className="mt-8 flex flex-wrap gap-2">
          {CITIES.map((city) => (
            <span key={city.slug} className="rounded-full border border-ink/15 px-4 py-2 text-sm font-medium">
              {city.name}
            </span>
          ))}
          <span className="rounded-full border border-dashed border-ink/25 px-4 py-2 text-sm text-ink/50">
            Your city next?
          </span>
        </div>
