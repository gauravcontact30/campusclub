          ...
          {bookings.map((booking) => {
            ...
            return (
              <li key={booking.id} className="surface-card flex flex-col gap-5 p-6 sm:flex-row sm:items-center">
                <div className="flex h-20 w-20 shrink-0 flex-col items-center justify-center rounded-2xl border border-frost/12 bg-noir-600 text-frost">
                  <span className="text-[10px] uppercase tracking-widest text-frost/60">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge tone={booking.status === 'confirmed' ? 'parrot' : 'zest'}>
                      {booking.status === 'confirmed' ? 'Confirmed' : 'Waitlisted'}
                    </Badge>
                  </div>

                  <p className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-frost/60">
                  ...
                  <p className="mt-2 text-sm text-frost/55">Seat fee {formatMoneyForCity(booking.event.priceCents, booking.event.city)}</p>
                </div>

                <div className="flex shrink-0 gap-2">
                  <Link
                    href={`/dinners/${booking.eventId}`}
                    className="inline-flex h-11 items-center rounded-full border border-frost/20 px-5 text-sm font-semibold hover:border-frost/45"
                  >
                    View table
                  </Link>
